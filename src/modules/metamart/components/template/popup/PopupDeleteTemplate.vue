<template>
  <base-popup name="popup-delete-template" class="popup-delete-banner" width="400px" :close="handleClose" :open="handleOpen">
    ...
    <div class="content">
      ...
      {{ $t('metamart.template.popup.delete-message-second') }}
    </div>
...
<script lang="ts">
  ...
  @Component
  export default class PopupDeleteTemplate extends Mixins(PopupMixin) {
    @Prop({ required: true, type: Object }) template!: Record<string, any>
    ...
    async handleConfirm(): Promise<void> {
      ...
      try {
        const permission = await this.checkPermission(this.template.id)
        ...
        if (permission === 1) {
          ...
          this.$message({
            message: this.$i18n.t('metamart.template.popup.do-not-delete') + '',
            duration: 3500,
            type: 'error'
          })
          this.setOpenPopup({
    ...
    async checkPermission(templateId: number): Promise<number> {
      try {
        await apiNft.getDeletePermission(templateId)
        return Promise.resolve(1)
      } catch (error: any) {
        if (error.statusCode === 400 && error.status === 'DONT_DELETE') {
