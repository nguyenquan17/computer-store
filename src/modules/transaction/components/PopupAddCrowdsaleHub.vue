<template>
  <base-popup name="popup-add-crowdsale-hub" class="popup-member" width="480px" :close="handleClose" :open="handleOpen">
    <div class="title-popup" slot="title">
      <span>{{ $t('transaction.popup.add-crowdsale', { coin: 'HUB' }) }}</span>
    </div>
    <div class="content" v-loading="isLoading">
      <el-form class="form-item" :model="form" :rules="rules" ref="setting-round-member" autocomplete="off">
        <el-form-item :label="$t('label.wallet-address')" :class="isEmailFailed ? 'is-error' : null" prop="tokenAddress">
          <el-input v-model="form.tokenAddress" :readonly="false" :placeholder="$t('placeholder.wallet-address')" clearable />
        </el-form-item>
        ...
        <el-form-item :label="$t('label.email')" prop="email">
          ...
          <el-input v-model="form.email" :readonly="false" :placeholder="$t('placeholder.email')" clearable />
        ...
        <div class="be-flex jc-space-between">
          <el-form-item :label="$t('label.paid-amount')" class="be-flex-item" :class="invalidAmount ? 'is-error' : null" prop="amount">
            <el-input
              ...
              :placeholder="$t('placeholder.amount-crowdsale', { coin: currency })"
    ...
    <div class="footer" slot="footer">
      <div class="be-flex wrap-button">
        <div class="btn-right">
          ...
          <el-button type="button" class="btn-default-bg text-sm ml-auto add-member" @click="handleSubmit" :disabled="!form.amount || !form.tokenAddress">
...
<script lang="ts">
  ...
  export default class PopupAddCrowdsaleHub extends Mixins(PopupMixin) {
    ...
    rules: Record<string, any> = {
      tokenAddress: [
        {
          required: true,
          message: this.$t('member.validate.wrong-wallet-address'),
          trigger: 'blur'
        }
      ],
      amount: [
    ...
    @Watch('form.amount') watchAmount(amount: string): void {
      const _amount = Number(String(this.form.amount)?.replaceAll(',', ''))
      this.form.tokenAmount = _amount * this.exchangeRate.oneSourceToTarget!
    }

    handleCancel(): void {
      this.setOpenPopup({
        popupName: 'popup-add-crowdsale-hub',
        isOpen: false
      })
    }
      ...
      this.$refs['setting-round-member']?.validate(valid => {
        if (valid && this.numClick === 1 && !this.invalidAmount && this.form.amount !== '0') {
          const submitForm = {
            ...
            isExternal: 1,
            paidAddress: this.form.tokenAddress,
            paidAmount: Number(String(this.form.amount)?.replaceAll(',', '')),
            paidCurrency: 'BUSD',
            paidNetwork: 'BEP20',
            paidSource: 'METAMASK',
            ...
            tokenAmount: Number(this.form.tokenAmount),
            tokenCurrency: 'HUB',
            tokenNetwork: 'BEP20',
