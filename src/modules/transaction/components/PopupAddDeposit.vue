        ...
        <div class="be-flex jc-space-between">
          <el-form-item :label="$t('label.amount')" class="be-flex-item" prop="amount">
            <el-input
              ...
              :placeholder="$t(`placeholder.amount-deposit`, { coin: currency })"
  ...
  export default class PopupAddDeposit extends Mixins(PopupMixin) {
    ...
    rules: Record<string, any> = {
      ...
      amount: [
        {
          required: true,
          message: this.$t('member.validate.wrong-deposit', { coin: this.currency }),
          trigger: 'blur'
        }
      ]
    }
    ...
    handleOpen(): void {
      this.isEmailFailed = false
      this.rules.amount[0].message = this.$t('member.validate.wrong-deposit', { coin: this.currency })
