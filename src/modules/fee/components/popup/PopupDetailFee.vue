<template>
  <base-popup name="popup-detail-fee" width="480px" class="popup-detail-fee" :is-show-footer="false">
    ...
    <div class="bg-[#fff] p-6 mb-2">
      ...
      <div class="flex justify-between border-b py-[14px] border-solid border-[#dbdbdb]">
        ...
        <div class="flex text-base text-[#0a0b0d]">
          <span>{{ useFormatTxCode(detailTransaction.transactionHash, 10) }}</span>
          <span class="ml-2" @click="useCopy(detailTransaction.transactionHash)">
      ...
      <div class="flex justify-between border-b py-[14px] border-solid border-[#dbdbdb]">
        ...
        <div class="flex text-base text-[#0a0b0d]">
          <span>{{ useFormatDateHourMs(detailTransaction.transactionDate) }}</span>
        </div>
      </div>
      <div class="flex justify-between border-b py-[14px] border-solid border-[#dbdbdb]">
        <p class="text-sm font-normal text-[#5b616e]">From</p>
        <div class="flex text-base text-[#0a0b0d]">
          <span>{{ useFormatTxCode(detailTransaction.fromAddress, 10) }}</span>
          <span class="ml-2" @click="useCopy(detailTransaction.fromAddress)">
            <base-icon icon="icon-copy" size="24" color="#A19F9D" />
          </span>
        </div>
      </div>
      ...
      <div class="flex justify-between border-b py-[14px] border-solid border-[#dbdbdb]">
        <p class="text-sm font-normal text-[#5b616e]">Fees</p>
        <div class="flex text-sm">
          <span>
            <span :class="checkValueAmountDisplay(detailTransaction.totalFeeDisplay)">{{
              detailTransaction.totalFeeDisplay
            }}</span>
            <span class="text-[#5b616e]"> (~${{ useFormatCurrency(detailTransaction.totalFeeToUsd, 'USD') }})</span>
          </span>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  interface IProps {
    detailTransaction: IFee
  }
  const props = withDefaults(defineProps<IProps>(), {
    detailTransaction: () => ({} as IFee)
  })
