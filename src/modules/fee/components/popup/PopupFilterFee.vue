<template>
  <base-popup name="popup-filter-fee" width="600px" class="popup-filter-fee" :is-show-footer="false" @open="handleOpen">
    ...
    <el-form label-position="top">
      ...
      <div class="flex justify-between">
        <el-form-item class="flex-1 mr-10" label="Transaction Fee">
          <el-input
            v-model="filter.fromTransactionFee"
            placeholder="From fee"
            @keypress="useOnlyNumber($event, filter.fromTransactionFee)"
            @keyup="useFormatNumberInput($event)"
          >
            <template #prefix>
              <div class="text-base text-[#0a0b0d]">$</div>
        ...
        <el-form-item class="flex-1 hide-label" label="1">
          <el-input
            v-model="filter.toTransactionFee"
            placeholder="To fee"
            @keypress="useOnlyNumber($event, filter.toTransactionFee)"
            @keyup="useFormatNumberInput($event)"
          >
            <template #prefix>
              <div class="text-base text-[#0a0b0d]">$</div>
...
<script setup lang="ts">
  ...
  const handleOpen = () => {
    ...
    filter.value.status = props.status
  }
